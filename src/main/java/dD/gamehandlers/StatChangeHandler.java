...
public class StatChangeHandler {
    ...
    public void parseStatChange(String[] statChanges) {
        ...
        } else if (statChanges[1].equals("-1D6+2")) {
            amount = 0 - die.rollOneDie() + 2;
        } else if (statChanges[1].equals("-1D6*2")) {
            amount = 0 - die.rollOneDie() * 2;
        } else if (statChanges[1].equals("-2D6")) {
        ...
        switch (statChanges[2]) {
            case "STAM":
                playerCharacter.setStamina(playerCharacter.getStamina() + amount);
                System.out.println("Player stamina affected : " + amount);
                break;

            case "SKILL":
                playerCharacter.setSkill(playerCharacter.getSkill() + amount);
                System.out.println("Player skill affected : " + amount);
                break;

            case "LUCK":
                playerCharacter.setLuck(playerCharacter.getLuck() + amount);
